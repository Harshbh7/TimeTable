    ...
    const handleBulkUpload = (data) => {
        ...
        data.forEach(row => {
            const payload = {
                ...
                initial: (row['Initials'] || row['initials'] || row['initial'] || '').toUpperCase(),
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 relative">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <input required name="name" placeholder="Teacher Name" value={formData.name} onChange={handleChange} className="border p-2 rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                <input required name="initial" placeholder="Initials (e.g. JD)" value={formData.initial} onChange={handleChange} className="border p-2 rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                ...
                <table className="w-full text-left">
                    <thead className="border-b dark:border-gray-700">
                        <tr>
                            ...
                            <th className="p-3 dark:text-gray-300">Initials</th>
                    ...
                    <tbody>
                        {Object.entries(teachers).map(([id, teacher]) => (
                            <tr key={id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-750">
                                <td className="p-3 dark:text-gray-300">{teacher.name}</td>
                                <td className="p-3 dark:text-gray-300">{teacher.initial}</td>
            ...
            <BulkUploadModal
                ...
                entityName="Teachers"
                expectedColumns={['Name', 'Initials', 'Department']}
            />
        </div>
    );
};
