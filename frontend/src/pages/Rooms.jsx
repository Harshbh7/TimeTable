...
const Rooms = () => {
    ...
    const [formData, setFormData] = useState({ name: '', type: 'Classroom', capacity: '' });
    ...
    const handleSubmit = (e) => {
        ...
        setFormData({ name: '', type: 'Classroom', capacity: '' });
    };
    ...
    const handleBulkUpload = (data) => {
        data.forEach(row => {
            const payload = {
                name: row['Name'] || row['name'] || row['Room Name'] || '',
                type: row['Type'] || row['type'] || 'Classroom',
                capacity: parseInt(row['Capacity'] || row['capacity'] || 0, 10)
            };
            if (payload.name) {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 relative">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <input required name="name" placeholder="Room Name (e.g. 101)" value={formData.name} onChange={handleChange} className="border p-2 rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                <select required name="type" value={formData.type} onChange={handleChange} className="border p-2 rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    <option value="Classroom">Classroom</option>
                    <option value="Lab">Lab</option>
                </select>
                <input required type="number" name="capacity" placeholder="Capacity" value={formData.capacity} onChange={handleChange} className="border p-2 rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    ...
                    <tbody>
                        {Object.entries(rooms).map(([id, room]) => (
                            <tr key={id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-750">
                                <td className="p-3 dark:text-gray-300">{room.name}</td>
                                <td className="p-3 dark:text-gray-300">
                                    <span className={`px-2 py-1 rounded text-xs font-semibold ${room.type === 'Lab' ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700'}`}>
                                        {room.type}
                                    </span>
                                </td>
                                <td className="p-3 dark:text-gray-300">{room.capacity}</td>
            ...
            <BulkUploadModal
                ...
                entityName="Rooms"
                expectedColumns={['Name', 'Type', 'Capacity']}
            />
        </div>
    );
};
