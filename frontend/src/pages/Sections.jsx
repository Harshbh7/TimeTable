...
const Sections = () => {
    ...
    const handleEdit = (id, section) => {
        setFormData({ name: section.name || '', studentCount: section.studentCount || 70, semester: section.semester || '' });
        setEditingId(id);
    };
    ...
    const handleBulkUpload = (data) => {
        data.forEach(row => {
            const payload = {
                name: row['Name'] || row['name'] || row['Section Name'] || '',
                studentCount: parseInt(row['Students'] || row['students'] || row['Total Students'] || 70, 10),
                semester: row['Semester'] || row['semester'] || row['Sem'] || ''
            };
            if (payload.name) push(ref(db, 'sections'), payload);
        });
    };
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 relative">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                ...
                />
                <input
                    required type="number" name="studentCount"
                ...
                <table className="w-full text-left">
                    ...
                    <tbody>
                        {Object.entries(sections).map(([id, section]) => {
                            const half = Math.ceil((section.studentCount || 70) / 2);
                            const remainder = (section.studentCount || 70) - half;
                            return (
                                <tr key={id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-750">
                                    <td className="p-3 dark:text-gray-300 font-medium">{section.name}</td>
                                    <td className="p-3">
                                        {section.semester
                                    ...
                                    <td className="p-3 dark:text-gray-300">{section.studentCount}</td>
            ...
            <BulkUploadModal
                ...
                entityName="Sections"
                expectedColumns={['Name', 'Semester', 'Students']}
            />
        </div>
    );
};
